...
@objcMembers public class Throw: NSObject {
    ...
    /// A method with a parameter that can throw an error and return a value using typed `throws(ThrowError)`.
    ...
    public func throwAnErrorWithParamAndReturn(input: String) throws(ThrowError) -> String {
        if input.isEmpty {
